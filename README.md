...
- **Security perimeter:** Led the ModSecurity v3 deployment on Nginx with the OWASP CRS tuned to avoid false positives on REST, JSON, OAuth, and Socket.io traffic, while enforcing rate limits at both Nginx (5 req/min auth, 120 req/min APIs) and Express layers. Helmet headers, strict CORS, and secrets fetched from HashiCorp Vault protect every facet of the HTTP layer.
