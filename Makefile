...
help:
	...
	@echo "  dev      - Run docker compose with .env.development"
	@echo "  prod     - Run docker compose with .env"
...
dev:
	...
	make ensure-secrets && \
	docker compose -p $(PROJECT_NAME) -f $(DOCKER) --env-file .env.development up -d --build
...
re:
	...
	make ensure-secrets && \
	docker compose -p $(PROJECT_NAME) -f $(DOCKER) --env-file .env.development up -d --build --force-recreate

clean:
	@echo "Taking down containers but leaving volumes intact..." && \
	docker compose -p $(PROJECT_NAME) -f $(DOCKER) --env-file .env.development down

fclean:
	@echo "Taking down containers and purging volumes and networks..." && \
	docker compose -p $(PROJECT_NAME) -f $(DOCKER) --env-file .env.development down -v --rmi all && \
...
migrate:
	...
	make ensure-secrets && \
	docker compose -p $(PROJECT_NAME) -f $(DOCKER) --env-file .env.development run --rm -e NODE_ENV=migration -e MIGRATION_NAME=$(name) backend
